...
(function(){
	...
	prop._drawEle = function(x,y,cN){
		var div = $('<div>');
		cN && div.addClass(cN);
		return div.css(getCoords(x,y)).appendTo(this.html);
	}
	/*放棋子*/
	prop._putChess = function(x,y,robot){
		console.log('putChess:',x,y);
		var _this = this,
			pos = getPos(x,y,_this.BORDER_SIZE);
		if(_this.data[pos] > 0){
			return;
		}
		_this._drawEleXY(x,y,'ab chessman '+(_this.currentPlayer == PLAYER_WHITE?'white':'black'),-10,-10).html(++_this.chessNum);
		_this.data[pos] = _this.currentPlayer;
	...
	prop.check = function(x,y,continueFn){
		...
		function f(step_x,step_y){
			var _result = 0;
			for(var _x=x+step_x,_y=y+step_y;_x >= 0 && _x<bs && _y >= 0 && _y < bs;_x+=step_x,_y+=step_y){
				if(d[getPos(_x,_y,bs)] == player){
					_result++;
				}else{
					break;
				}
		...
		if(f(-1,0)+f(1,0) > 4 || f(0,-1)+f(0,1) > 4 || f(-1,-1)+f(1,1) > 4 || f(1,-0)+f(-1,-1) > 4){
	...
	prop.robot = function(x,y){
		...
		this._putChess.apply(this,this.brain.findBestStep(this.getData(),this.currentPlayer,this.BORDER_SIZE));
